...
chunksize = 10 ** 6
csvfile = open('situatii1.txt', 'r', newline = '')
readCSV = csv.reader(csvfile, delimiter=',')

#data = pd.read_csv("situatii_2017.csv", chunksize=chunksize, nrows=22)
data = pd.read_csv("situatii_2017.csv")
print (data.shape)

situatii_cols = 22
situatii_rows = 604731

row_iterator = data.iterrows()
#print (next(row_iterator))

def match_code1(code_cui, data):
    for index in range(0, 604732):
        try:
...
def match_code_1():
    with open('final2017_1.csv') as csvfile:
        readCSV = csv.reader(csvfile, delimiter='^')
        for row in readCSV:
            if len(row) is 6:
                result = []
                ...
                stare_firma = row[4]
                adresa = row[5]
                #find_result = match_code1(code_cui, data)
                ...
                if find_result:
                    ...
                    result.append(pierdere_neta)
                    result.append(nr_salariati)
                    with open('2017.csv', 'a+', newline='') as myfile:
                        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
                        wr.writerow(result)
                    myfile.close()

def match_code_2():
    with open('final2017_2.csv') as csvfile:
        readCSV = csv.reader(csvfile, delimiter='^')
        for row in readCSV:            
            if len(row) is 6:
                result = []
...
def match_code_3():
    with open('final2017_3.csv') as csvfile:
        readCSV = csv.reader(csvfile, delimiter='^')
        for row in readCSV:            
            if len(row) is 6:
                result = []
...
def match_code_4():
    with open('final2017_4.csv') as csvfile:
        readCSV = csv.reader(csvfile, delimiter='^')
        for row in readCSV:            
            if len(row) is 6:
                result = []
